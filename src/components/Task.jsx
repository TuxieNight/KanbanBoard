...
function formatRelativeTime(value) {
    ...
    return seconds > 0 ? `${amount} ${unit.label}${plural} ago` : `in ${amount} ${unit.label}${plural}`;
}
...
export default function Task(props) {
    ...
    const priorityClass = props.priority ? `priority-tag priority-${String(props.priority).toLowerCase()}` : 'priority-tag';
    ...
    return (<Card className="task-card">
        <Container>
            ...
            <p><em>Posted: {createdAt}</em></p>
            <p>{props.description}</p>
            {
                dueDate != '' && <p><strong>Due: {dueDate}</strong></p>
            }
            {props.assignee_id}
        </Container>
    </Card>);
